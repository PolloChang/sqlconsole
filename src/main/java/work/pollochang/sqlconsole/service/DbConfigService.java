...
public class DbConfigService {
    ...
    public DbConfig saveConfig(DbConfig config) {
        if (config.getId() != null) {
            DbConfig existing = dbConfigRepository.findById(config.getId())
            ...
            if (config.getDbPassword() != null && !config.getDbPassword().equals("******") && !config.getDbPassword().isEmpty()) {
    ...
    /**
     * Creates a raw JDBC connection for the given config.
     * Decrypts credentials before connecting.
     */
    public Connection createConnection(DbConfig config) throws SQLException {
        ...
        return createConnection(config.getDbType(), config.getJdbcUrl(), decryptedUser, decryptedPassword);
    }

    /**
     * Helper to create connection with plain text credentials.
     * Handles Oracle roles (e.g. "sys as sysdba").
     */
    public Connection createConnection(DbType dbType, String url, String user, String password) throws SQLException {
        ...
        if (user != null) {
            ...
            if (dbType == DbType.ORACLE) {
                ...
                Pattern p = Pattern.compile("^(.*)\\s+as\\s+(sysdba|sysoper|sysasm)$", Pattern.CASE_INSENSITIVE);
